...
export function MessageCard({ message, zoneType, isDragging }: MessageCardProps) {
    ...
    // Mock data for aesthetics matching the image
    const initials = message.sender ? message.sender.split(' ').map((n: string) => n[0]).join('') : 'AI';
    const timeAgo = '5m ago';
    const isUrgent = isInstant && message.id.includes('1'); // Just for visualVariety
        ...
        return (
            ...
            >
                ...
                <div className="flex justify-between items-start mb-4 text-left relative z-10">
                    ...
                    <span className="text-[10px] font-mono font-bold text-[#BEF264] bg-[#BEF264]/10 px-2 py-0.5 rounded border border-[#BEF264]/20">78%</span>
                </div>
                ...
                <div className="flex justify-end relative z-10">
                    <button className="flex items-center gap-2 px-5 py-2.5 bg-[#BEF264] hover:bg-[#D9FA9E] text-black rounded-xl text-[10px] font-bold transition-all shadow-[0_0_20px_rgba(190,242,100,0.2)] active:scale-95">
                    ...
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                            <h4 className="text-[13px] font-bold text-zinc-300 group-hover:text-white transition-colors">
                                {message.sender ? message.sender.split(' ')[0] : "Team"}...
                            </h4>
                            <span className="px-2 py-0.5 rounded border border-zinc-800 bg-[#0A0A0A] text-[8px] font-bold text-zinc-500 uppercase tracking-widest">
                                {message.id.includes('2') ? 'CONFIRMED' : 'QUEUED'}
                            </span>
                        </div>
                        <span className="text-[10px] font-mono text-zinc-600 font-bold">15:00</span>
                    </div>
    ...
    return (
        ...
        >
            {/* Instant Glow Effect - Mouse Following Simulation via group-hover */}
            ...
            <div className="flex justify-between items-start mb-5 relative z-10">
                ...
                <button className="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-700 hover:text-zinc-300 hover:bg-[#151515] transition-all">
